...
export async function runQualityManager(jobId: string) {
  ...
  // Fetch unverified raw records from this job
  ...
  for (const record of rawRecords) {
    ...
    // Insert or update into verified_businesses
  ...
  // Update job with verified count
  await supabase
    .from('agent_jobs')
    .update({ records_verified: verifiedCount })
    .eq('id', jobId);
